//! dsh server manager.
//!
//! Responsibilities (mirroring the Electron `dsh-desktop` client):
...
//! - probe `127.0.0.1:3080` (plus any extra ports from the web profile's
//!   patch layer) and attach to an already-running harness instead
//!   of spawning a second instance (avoids concurrent writers on `~/.dsh`)
//! - connection modes: **smart** (attach to an existing harness, else start a
//!   local runtime) vs **fixed address** (connect to a user-configured URL,
//!   never start anything) — settings persist in `~/.dsh-desktop/settings.json`
//! - runtime lock: record the spawned child in `~/.dsh/.dsh-desktop-runtime.json`
//!   so a next start adopts a surviving child instead of spawning beside it
...
// ── connection settings (mirrors Electron's ~/.dsh-desktop/settings.json) ──

/// Persisted client settings, matching the Electron version's schema.
#[derive(Clone, Serialize, Deserialize)]
#[serde(rename_all = "camelCase")]
pub struct ConnectionSettings {
    #[serde(skip_serializing_if = "Option::is_none")]
    pub server_url: Option<String>,
    /// "smart" (default) or "connect" (fixed address).
    ...
    #[serde(skip_serializing_if = "Option::is_none")]
    pub bundled_market_disabled: Option<bool>,
}
...
impl Default for ConnectionSettings {
    fn default() -> Self {
        Self {
            server_url: None,
            connection_mode: "smart".to_string(),
            update_dismissed_version: None,
            update_last_checked_at: None,
            bundled_market_disabled: None,
        }
    }
}

/// The client's own data home (connection settings only).
pub fn dsh_desktop_home_dir(app: &AppHandle) -> PathBuf {
    if let Some(dir) = env_nonempty("DSH_DESKTOP_HOME") {
        return PathBuf::from(dir);
    }
    resolve_home(app).join(".dsh-desktop")
}

fn settings_file_path(app: &AppHandle) -> PathBuf {
    dsh_desktop_home_dir(app).join("settings.json")
}

pub fn load_settings(app: &AppHandle) -> ConnectionSettings {
...
pub fn save_settings(app: &AppHandle, settings: &ConnectionSettings) -> Result<(), String> {
    let path = settings_file_path(app);
    if let Some(dir) = path.parent() {
...
// ── runtime lock (mirrors Electron's ~/.dsh/.dsh-desktop-runtime.json) ─────

/// Records the spawned dsh child so the next start can adopt it instead of
/// spawning a second writer on the same `~/.dsh`.
#[derive(Clone, Serialize, Deserialize)]
#[serde(rename_all = "camelCase")]
struct RuntimeLock {
    child_pid: u32,
...
fn runtime_lock_file(app: &AppHandle) -> PathBuf {
    dsh_home_dir(app).join(".dsh-desktop-runtime.json")
}

fn read_runtime_lock(app: &AppHandle) -> Option<RuntimeLock> {
    let path = runtime_lock_file(app);
    let content = fs::read_to_string(path).ok()?;
    serde_json::from_str(&content).ok()
}

fn write_runtime_lock(app: &AppHandle, lock: &RuntimeLock) {
    let path = runtime_lock_file(app);
    ...
    if let Ok(content) = serde_json::to_string(lock) {
        if fs::write(&tmp, &content).is_ok() {
            ...
            // remove the old advisory record first, then move the complete
            // temporary document into place.
...
fn clear_runtime_lock(app: &AppHandle) {
    let _ = fs::remove_file(runtime_lock_file(app));
}

fn clear_runtime_lock_for_pid(app: &AppHandle, pid: u32) {
    if read_runtime_lock(app).is_some_and(|lock| lock.child_pid == pid) {
...
/// npm's npx cache root, matching the Electron client: `%LOCALAPPDATA%\\npm-cache\\_npx`
/// on Windows, `~/.npm/_npx` elsewhere, with npm_config_cache taking precedence.
fn npx_cache_root(app: &AppHandle) -> Option<PathBuf> {
    if let Some(cache) = env_nonempty("npm_config_cache").or_else(|| env_nonempty("NPM_CONFIG_CACHE")) {
        return Some(PathBuf::from(cache).join("_npx"));
    }
    if cfg!(target_os = "windows") {
        return env_nonempty("LOCALAPPDATA")
            .map(|dir| PathBuf::from(dir).join("npm-cache").join("_npx"));
    }
    Some(resolve_home(app).join(".npm").join("_npx"))
}

/// Finds the official dsh bin inside any npx spec cache entry. Directory order
/// is not a contract, so every candidate is validated by its actual bin path.
fn npx_cached_bin(app: &AppHandle) -> Option<PathBuf> {
    let root = npx_cache_root(app)?;
    let entries = fs::read_dir(root).ok()?;
    for entry in entries.flatten() {
        let candidate = runtime_bin_path(&entry.path());
...
fn global_dsh_bin() -> Option<PathBuf> {
    let mut cmd = npm_ecosystem_command("npm", &["root", "-g"]);
...
fn resolve_bin(app: &AppHandle, server: &Shared) -> Result<String, String> {
    ...
    if let Some(global) = global_dsh_bin() {
        let value = plain_win_path(&global);
        push_log(server, format!("复用全局安装的 dsh: {value}"));
        return Ok(value);
    }
    // Reuse a package already downloaded by `npx @deepseek-ai/dsh web`
    // before falling back to the bundled or managed runtime. This mirrors the
    // Electron client's developer-friendly smart mode and avoids a download.
    if let Some(cached) = npx_cached_bin(app) {
        let value = plain_win_path(&cached);
        push_log(server, format!("复用 npx 缓存中的 dsh: {value}"));
        return Ok(value);
    }
    // bundled runtime shipped with packaged builds
    if let Ok(res) = app.path().resource_dir() {
...
fn set_running(app: &AppHandle, server: &Shared, url: &str) {
    ...
    if let Some(mut lock) = read_runtime_lock(app) {
        let current_pid = server.lock().unwrap().pid;
        if current_pid == Some(lock.child_pid) {
            lock.url = Some(url.to_string());
            write_runtime_lock(app, &lock);
        }
    }
    {
        let mut s = server.lock().unwrap();
...
fn spawn(app: &AppHandle, server: &Shared, node: &str, bin: &str, port: u16) -> Result<(), String> {
    ...
    // Write before readiness so a client killed during bootstrap can adopt the
    // surviving child on its next launch, matching the Electron client.
    write_runtime_lock(
        app,
        &RuntimeLock {
            child_pid: pid,
    ...
    thread::spawn(move || {
        ...
        clear_runtime_lock_for_pid(&app3, pid);
        if requested {
...
pub fn restart(app: &AppHandle, server: &Shared) {
    stop(app, server);
    // stop() clears the child pid, but the old status can still be Running
    // until its watcher reaps the process. Mark it stopped immediately so
    // start() does not treat the restart as a no-op.
    set_stopped(app, server);
...
/// Ports configured by the official web profile patch layer. The patch file is
/// intentionally parsed loosely, matching Electron's discovery logic: a wrong
/// candidate costs one quick probe, while missing a real candidate could start
/// a second writer against the same `~/.dsh`.
fn configured_web_ports(app: &AppHandle) -> Vec<u16> {
    let patch = dsh_home_dir(app)
        .join("profiles")
        .join("web")
        .join("cordis.patch.yml");
    let Ok(source) = fs::read_to_string(patch) else {
        return Vec::new();
    };
    let mut ports = Vec::new();
    for line in source.lines() {
        let trimmed = line.trim();
        let Some(value) = trimmed.strip_prefix("port:") else {
            continue;
        };
...
fn smart_probe_origins(app: &AppHandle) -> Vec<String> {
    let default = default_port();
    let mut origins = vec![format!("http://127.0.0.1:{default}")];
    for port in configured_web_ports(app) {
...
pub fn start(app: &AppHandle, server: &Shared) -> Result<(), String> {
    ...
    // Fixed-address mode is intentionally a pure connection path: it never
    // resolves Node, installs dsh, or starts a second writer in ~/.dsh.
    ...
    if settings.connection_mode == "connect" {
        let url = settings
            ...
            .ok_or_else(|| "固定地址模式需要有效的 Web UI 地址（http:// 或 https://）。".to_string())?;
        push_log(server, format!("固定地址模式：连接 {url}"));
...
fn start_inner(app: &AppHandle, server: &Shared) -> Result<(), String> {
    ...
    // Attach to any already-running harness origin before resolving
    // node/runtime. This includes ports configured in the official profile
    // patch, not only the default 3080.
    let port = default_port();
    let probe_origins = smart_probe_origins(app);
    for origin in &probe_origins {
        if probe_dsh_with_retries(origin, 2, 250) {
            push_log(server, format!("检测到已在运行的 dsh 服务，直接使用 {origin}"));
            set_running(app, server, origin);
            return Ok(());
        }
    }
    ...
    loop {
        ...
        if exited && eaddr {
            ...
            for origin in smart_probe_origins(app) {
                if probe_dsh_with_retries(&origin, 3, 400) {
                    push_log(
                        server,
                        format!("端口占用者是 dsh 服务（初始探测超时），直接使用 {origin}…"),
                    );
                    set_running(app, server, &origin);
                    return Ok(());
                }
...
pub fn info(app: &AppHandle, server: &Shared) -> serde_json::Value {
    ...
    let settings = load_settings(app);
    let runtime_source = match bin.as_deref() {
        Some(value) if value.contains("npm-cache") || value.contains("_npx") => "npx",
        Some(value) if value.contains("node_modules") => "global-or-path",
        Some(value) if value.contains("runtime") => "bundled-or-managed",
        Some(_) => "override-or-path",
        None => "unknown",
    };
    serde_json::json!({
...
mod tests {
    ...
    #[test]
    fn configured_web_ports_are_parsed_and_deduplicated() {
        let root = std::env::temp_dir().join(format!("dsh-patch-test-{}", std::process::id()));
        let _ = fs::create_dir_all(&root);
        let patch = root.join("cordis.patch.yml");
        fs::write(&patch, "port: 3080\nport: '4096'\nport: 3080\nport: 70000\n").unwrap();
        let source = fs::read_to_string(&patch).unwrap();
        let mut ports = Vec::new();
        for line in source.lines() {
            let Some(value) = line.trim().strip_prefix("port:") else { continue };
            let Ok(port) = value.trim().trim_matches(|c| c == '\'' || c == '"').parse::<u16>() else { continue };
            if port > 0 && !ports.contains(&port) { ports.push(port); }
        }
        assert_eq!(ports, vec![3080, 4096]);
