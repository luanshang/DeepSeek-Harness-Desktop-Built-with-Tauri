...
| `DSH_DESKTOP_HOME` | 桌面端连接设置目录（默认 `~/.dsh-desktop`） |

### 连接模式

桌面端连接设置保存在 `~/.dsh-desktop/settings.json`，使用与 Electron 版兼容的 camelCase 字段：

- `connectionMode: "smart"`：依次接管运行时锁、探测默认端口和 profile patch 中的端口、复用全局 dsh、复用 npx 缓存，最后才使用内置或托管运行时。
- `connectionMode: "connect"`：使用 `serverUrl` 连接已有 Web UI，不启动 Node/dsh 子进程，适合自行维护的服务。
- 连接设置可从无边框窗口的汉堡菜单、系统托盘菜单或 macOS「文件」菜单打开。

客户端启动的 dsh 子进程会在 `~/.dsh/.dsh-desktop-runtime.json` 写入 advisory runtime lock。下次启动会先验证锁中 URL 是否仍可达，成功后接管而不是再启动一个写入同一 `DSH_HOME` 的服务；失效锁会自动清理。
...
最终用户安装器使用项目内独立的 Catppuccin Mocha WebView2 UI：

- `installer/ui/index.html`：安装流程
- `installer/ui/styles.css`：Catppuccin Mocha 样式
- `installer/ui/app.js`：安装器交互
- `installer-host/`：独立 WebView2/Tauri 安装器宿主
- `src-tauri/icons/icon.ico`：安装器图标
- 安装器支持“仅为当前用户安装”和“为所有用户安装”两种范围

```bash
npm install
npm run bundle        # fetch:node → prepare:runtime → CSS WebView2 installer build
# 最终输出：src-tauri/target/release/bundle/nsis/DeepSeek Harness Desktop_<version>_x64-setup.exe
# 或者分步执行：
...
npm run build         # → src-tauri/target/release/bundle/nsis/DeepSeek Harness Desktop_<version>_x64-setup.exe
```
